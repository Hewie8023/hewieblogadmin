...
public class EmailSender {
    ...
    private EmailSender() {
        EmailSender.config(EmailSender.SMTP_163(false), "hewie802326@163.com", "BMAVIDKEJUEQXWOU");
    }
    ...
    public static void sendCommentNotify(String notifyContent, String emailAddress) throws Exception {
        EmailSender.subject("Hewie博客系统评论通知")
                .from("Hewie博客系统")
                .text(notifyContent)
                .to(emailAddress)
                .send();
    ...
    public static void sendRegisterVerifyCode(String code, String emailAddress) throws Exception {
        EmailSender.subject("Hewie博客系统注册验证码")
                .from("Hewie博客系统")
                .text("您的验证码是：" + code + "（验证码有效期为2分钟，若非本人操作，请忽略！！！）")
                .to(emailAddress)
                .send();
    }
}
